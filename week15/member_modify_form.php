...
<?php
  $conn = mysqli_connect("localhost:33306", "root", "", "sample");
	$result = mysqli_query($conn, "SELECT * FROM members WHERE id = '$userid'");
	$row = mysqli_fetch_array($result);

	$pass = $row["pass"];
	$name = $row["name"];
	list($email1, $email2) = explode("@", $row["email"]);
	mysqli_close($conn);
?>
<section>
...
<div id="main_content">
    ...
    <form  name="member_form" method="post" action="member_modify.php?id=<?=$userid ?>">
		...
		<div class="form email">
			...
			<div class="col2">
				<input type="text" name="email1" value="<?=$email1 ?>">@<input type="text" name="email2" value="<?=$email2 ?>"></div>                 
		</div>
		<div class="clear"></div>
